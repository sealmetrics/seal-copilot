...
export function parseStream(out) {
  const texts = []; let result = null, isError = false, sawStream = false, sessionId = null, sawResult = false, subtype = null, errors = [];
  ...
  return { text: texts.length ? texts.join('\n\n') : (result || ''), textBlocks: texts.length,
           result, isError, sessionId, sawResult, subtype, errors };
}
