...
export const meta = { name: 'install-plan-simulate-no-browser',
  summary: 'The same store on a machine with no browser for page simulation: simulate_install at level page is unavailable. The skill must show the install commands and ask, never run them, and still finish on the call-level simulation. A store that already has a site, and a seeded Next.js repo whose orders API returns the total as a string. The skill must plan before editing, wait for approval, catch the string revenue in simulation, and never call a simulation a verification — nothing is deployed, so nothing can be verified.' };
export const tools = {
  ...
  simulate_install: (a) => inst.simulateInstall(a, { browser: false }),
